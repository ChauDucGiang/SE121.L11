        ...
        <vs-dropdown-menu class="cart-dropdown vx-navbar-dropdown" :class="{'dropdown-custom': cartItems.length}">
            ...
            <template v-if="cartItems.length">
                <div class="notification-header text-center p-5 bg-primary text-white">
                    ...
                    <p class="opacity-75">In Your Selectd</p>
                </div>
                ...
                <component :is="scrollbarTag" ref="mainSidebarPs" class="scroll-area--cart-items-dropdowm p-0 mb-10"
                    ...
                    <ul class="bordered-items">
                        <li
                            ...
                            @click="changeProductDetail(item.id)">

                            <!-- IMG COL -->
                            <div class="vx-col w-1/5 item-img-container bg-white flex items-center justify-center">
                                <img :src="item.images[0]['url']" :alt="item.name" class="cart-dropdown-item-img p-4">
                            </div>
                ...
                <div
                    ...
                    @click="$router.push({name: 'Cart'}).catch(() => {})">

                <span class="flex items-center justify-center">
                  <feather-icon icon="ShoppingCartIcon" svgClasses="h-4 w-4"/>
                  <span class="ml-2">View Selected</span>
                </span>
            ...
            <!-- IF CART IS EMPTY -->
            <template v-else>
                <p class="p-4">Your Selected Is Empty.</p>
            </template>
        </vs-dropdown-menu>
    </vs-dropdown>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            changeProductDetail(id) {
                ...
                // this.$store.dispatch('home/FETCH_PRODUCT', id).catch(err => {
                //     console.error(err)
                // });
                EventBus.$emit('changeProductDetail', params);
            },
        }
    }
