...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            isGroupActive() {
                return (item) => {
                    const path = this.$route.query.category_id;
                    let open = false
                    // const routeParent = this.$route.meta ? this.$route.query.category_id : undefined
                    const routeParent = this.$route.query.category_id

                    const func = (item) => {
                        if (item.submenu) {
                            item.submenu.forEach((item) => {
                                if ((path === item.id || routeParent === item.slug) && item.id) {
                                    open = true
