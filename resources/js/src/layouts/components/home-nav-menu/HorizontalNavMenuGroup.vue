      ...
      {'disabled-item pointer-events-none': group.isDisabled }
        ...
        <transition name="fade-bottom-2x">
            <ul :style="styleItems" class="h-nav-group-items h-nav-menu-dd absolute shadow-drop py-2" v-show="openItems"
                ref="childDropdown">
                <li class="subcategory" v-for="(groupItem, index) in group.subcategories" :key="index"
                    @click="changeCategory(group.id,groupItem.id)">

                    <h-nav-menu-group
                        v-if="groupItem.subcategories"
                        :group="groupItem"
...
<script>
    ...
    export default {
        ...
        methods: {
            changeCategory(category_id, subcategory_id) {
                let query = {
